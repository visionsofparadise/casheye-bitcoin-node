...
let config: any = {
	...
	blocknotify: `curl -d '{"timestamp": "$(date -Ins)"}' -X POST "http://localhost:3000/new-block/%s"`,
	walletnotify: `curl -d '{"timestamp": "$(date -Ins)"}' -X POST "http://localhost:3000/new-tx/%s"`,
};
